...
from resources.bandits import AgentNetwork, get_update_dynamics
...
def define_sindy_configuration():
    ...
    return rnn_modules, control_parameters, library_setup, filter_setup

def objective(trial, train_dataset, n_participants):
    """
    ...
    try:
        ...
        #     ys_val = val_dataset.xs[mask, :, :agent_spice._n_actions]
...
def evaluate_with_sindy(model_rnn, val_dataset, participant_ids, n_participants, best_params):
    ...
    for pid in participant_ids:
        ...
        try:
            ...
            sindy_params = participant_sindy.count_parameters(mapping_modules_values={'x_learning_rate_reward': 'x_value_reward', 'x_value_reward_not_chosen': 'x_value_reward', 'x_value_choice_chosen': 'x_value_choice', 'x_value_choice_not_chosen': 'x_value_choice'})[pid] if hasattr(participant_sindy, 'count_parameters') else 0
...
def main():
    ...
    for data_file in os.listdir(data_dir):
        ...
        dataset = convert_dataset(file=data_file)[0]
        
        # Split trials within each participant
        train_dataset, val_dataset = split_data_along_timedim(dataset, split_ratio=train_test_ratio)
        participant_ids = train_dataset.xs[:, 0, -1].unique()
        ...
        objective_func = lambda trial: objective(trial, train_dataset, n_participants)
