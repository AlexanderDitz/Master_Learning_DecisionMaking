...
data_path = '/Users/martynaplomecka/closedloop_RL_new/closedloop_rl/data/study_recovery_stepperseverance/data_rldm_128p_0.csv'
#data_path = '/Users/martynaplomecka/closedloop_RL_new/closedloop_rl/data/study_recovery_stepperseverance/data_rldm_8p_0.csv'
...
for i, participant_id in enumerate(unique_participants):
    ...
    # Build input tensor xs of shape [1, n_trials, 5]
    xs = torch.zeros((1, n_trials, 5))
    for t in range(1, n_trials):
        prev_choice = participant_df['choice'].iloc[t-1]
        xs[0, t, int(prev_choice)] = 1.0  # one-hot encoding of previous action
        xs[0, t, 2] = participant_df['reward'].iloc[t-1]  # previous reward for chosen action
        xs[0, t, 3] = -1  # fixed value for not-chosen action
    xs[0, :, 4] = i  # participant ID in the last column
    
    # Build output tensor ys (one-hot encoding of current choice)
...
# Combine data for all participants into a single dataset
combined_xs_full = torch.cat(all_xs)  # Shape: [n_participants, n_trials, 5]
combined_ys_full = torch.cat(all_ys)  # Shape: [n_participants, n_trials, n_actions]
...
def run_training_and_evaluation(dataset, label):
    ...
    # Train the RNN on the combined dataset
    logger.info("\nTraining RNN...")
    model_rnn = RLRNN(
        n_actions=n_actions, 
    ...
    # fit_model returns (model, optimizer, final_train_loss)
    ...
    agent_sindy = fit_model_sindy(
        ...
        optimizer_alpha=1,
        n_trials_off_policy=dataset.xs.shape[1],
        ...
        for pid in sorted(available_participant_ids):
            if (hasattr(agent_sindy._model, 'submodules_sindy') and 
                ...
                try:
                    ...
                    logger.error(f"Error accessing SINDy model for module {module_name} participant {pid}: {str(e)}")
            else:
