...
for i, participant_id in enumerate(unique_participants):
    ...
    for t in range(1, n_trials):
        ...
        # FIXED: Dynamically assign reward based on chosen action
        if int(prev_choice) == 0:
            xs[0, t, 2] = participant_df['reward'].iloc[t-1]  # reward for action 0
            xs[0, t, 3] = -1  # not chosen (action 1)
        else:  # prev_choice == 1
            xs[0, t, 2] = -1  # not chosen (action 0)
            xs[0, t, 3] = participant_df['reward'].iloc[t-1]  # reward for action 1
