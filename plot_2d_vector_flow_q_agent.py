...
parser = argparse.ArgumentParser(description='Plot 2D vector flow of Q-values for Q-agent.')
parser.add_argument('--csv', type=str, required=False, default=None,
                    help='Path to the generated_behavior_q_agent.csv file')
...
if args.csv is None:
    ...
    args.csv = next((os.path.join(synthetic_dir, f) for f in os.listdir(synthetic_dir)
                     if 'q_agent' in f and f.endswith('.csv')), None)
    if args.csv is None:
        raise FileNotFoundError('Could not find a q_agent synthetic data CSV file.')
