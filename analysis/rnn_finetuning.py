...
def split_dataset_by_participants(dataset, train_ratio=0.7):
    """
    Split dataset by participants rather than by trials.
    this way,we have completely different participants in train and validation sets.
    70/30 train/test split by default.
    """
    # Get unique participant IDs from the last feature dimension
    all_participant_ids = torch.unique(dataset.xs[:, 0, -1]).tolist()
    n_participants = len(all_participant_ids)
    
    logger.info(f"Total unique participants for splitting: {n_participants}")
    
    np.random.shuffle(all_participant_ids)
    
    split_idx = int(n_participants * train_ratio)
    train_ids = all_participant_ids[:split_idx]
    val_ids = all_participant_ids[split_idx:]
    
    logger.info(f"Train/test split: {train_ratio}/{1-train_ratio}")
    logger.info(f"Train participants: {len(train_ids)}, Validation participants: {len(val_ids)}")
    
    train_mask = torch.zeros(len(dataset), dtype=torch.bool)
    val_mask = torch.zeros(len(dataset), dtype=torch.bool)
    
    for i in range(len(dataset)):
        participant_id = dataset.xs[i, 0, -1].item()
        if participant_id in train_ids:
            train_mask[i] = True
        elif participant_id in val_ids:
            val_mask[i] = True
    
    logger.info(f"Train mask sum: {train_mask.sum()} sequences")
    logger.info(f"Validation mask sum: {val_mask.sum()} sequences")
    
    train_xs = dataset.xs[train_mask]
    train_ys = dataset.ys[train_mask]
    val_xs = dataset.xs[val_mask]
    val_ys = dataset.ys[val_mask]
    ...
    # Verify that participants are correctly separated
    train_participants = torch.unique(train_xs[:, 0, -1]).tolist()
    val_participants = torch.unique(val_xs[:, 0, -1]).tolist()
    
    return train_dataset, val_dataset, train_participants, val_participants
...
def objective(trial, train_dataset, val_dataset, n_participants):
    ...
    # Fit RNN with trial hyperparameters
    model_rnn = RLRNN(
        n_actions=n_actions,
    ...
    try:
        ...
        for pid in val_participant_ids:
            ...
            if probs.shape != choices_np.shape:
                logger.warning(f"Shape mismatch for participant {pid}: probs {probs.shape}, choices {choices_np.shape}")
                continue
        ...
        logger.info(f"Trial {trial.number}: Average Validation Loss: {avg_val_loss:.4f}")
...
def evaluate_with_sindy(model_rnn, train_dataset, val_dataset, val_participant_ids, n_participants):
    """
    Evaluate using SINDy by fitting separate models for validation participants.
    """
    ...
    logger.info("Evaluating with SINDy - fitting separate models for each validation participant")
    
    all_bic_values = []
    all_ll_values = []
    participant_equations = {}
    participant_metrics = {}  
    # Process each validation participant
    for pid in val_participant_ids:
        logger.info(f"Processing validation participant {pid}...")
        
        mask = (val_dataset.xs[:, 0, -1] == pid)
        if not torch.any(mask):
            logger.warning(f"No validation data for participant {pid}")
            continue
        ...
        try:
            ...
            values, probs, _ = get_update_dynamics(xs_val[0], participant_sindy)
            choices_np = xs_val[..., :participant_sindy._n_actions].cpu().numpy().squeeze(0)
            ...
            logger.info(f"Participant {pid}: LL={normalized_ll:.4f}, BIC={normalized_bic:.4f}, Params={sindy_params}")
    ...
    logger.info(f"Number of validation participants with valid BIC metrics: {len(all_bic_values)}/{len(val_participant_ids)}")
...
def main():
    ...
    for data_file in all_data_files:
        ...
        train_dataset, val_dataset, train_participant_ids, val_participant_ids = split_dataset_by_participants(combined_dataset, train_ratio=0.7)
        ...
        logger.info("OPTUNA!!!!! Starting hyperparameter optimization...")
        ...
        avg_bic, avg_ll, participant_equations, participant_metrics = evaluate_with_sindy(
            model_rnn, train_dataset, val_dataset, val_participant_ids, n_participants
        )
